...
函数*down_interruptible()*试图获取指定的信号量，如果不可用，则将进程设置为TASK_INTERRUPTIBLE——进入睡眠。这种进程状态意味着任务可以被信号唤醒。使用*down_trylock()*函数，在信号量已经被占用时，立刻返回非0值；否则它返回0，并且让你成功持有信号量。
...
在单核处理器系统中，优先级更高的任务可以抢占正在执行的优先级较低的任务。在多核处理器系统中，如果一个任务在持有自旋锁时被抢占，那么在其他核上运行的任务可能会因为等待这个锁而陷入死锁。

要解决以上问题，可以通过*preempt_disable()*禁止内核抢占。这是一个可以嵌套调用的函数，但每次调用都必须要有一个*preempt_enable()*释放。当最后一次preempt()被调用时，内核抢占才重新启用。抢占计数存放着preempt_enable()的调用次数，如果大于0，那么内核就不会进行抢占。可以使用*preempt_count()*返回这个值。
...
此类指令与体系结构密切相关，请参考[内存屏障指令](../../arm/barrier.md)。
