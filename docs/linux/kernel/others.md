...
Linux内核对LRU算法进行了改进，被称为双链策略——维护两个链表：活跃链表和不活跃链表。处于活跃链表上的页被认为是“热”的且不会换出，处于不活跃链表上的页被认为是“冷”的可以被换出。页面从尾部加入，从头部移除，就像队列一样。有了这两个LRU链表后，预读页就只需要加入到不活跃链表的头部，当页被真正访问的时候，才将页插入到活跃链表的头部。如果预读的页一直没有被访问，就会从不活跃链表中移除，这样就不会影响活跃链表中的热数据。
