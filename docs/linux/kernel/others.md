...
当`jiffies`变量的值超过它的最大存放范围后就会发生溢出，此时再继续增加的话，它会回绕至0。为了解决`jiffies`变量的溢出问题，内核引入了`jiffies_64`变量，这是一个64位的无符号整数。
...
基于`jiffies`的延迟方法受限于时钟节拍，无法提供更短、更精确的延迟要求。为此，内核提供了三个可以处理us、ns和ms级别的延迟函数：
