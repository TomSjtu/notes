...
UMA(一致性内存访问)是一种对称多处理器系统的内存访问架构，它指的是所有处理器对内存的访问具有相同的延迟。在UMA系统中，所有处理器共享同一总线或交叉点，可以直接访问共享内存，这意味着无论哪个处理器访问内存，延迟都是相同的，UMA适用于小规模的多处理器系统。

现代多核处理器大多采用了NUMA架构，即非一致性内存访问架构。在NUMA架构下，每个CPU都有自己的本地内存，称为NUMA节点(node)，它的访问速度更快。如果一个处理器要访问其他处理器的本地内存，需要通过QPI总线去访问，这样延迟就会增加。
