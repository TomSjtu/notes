...
现代多核处理器采用了NUMA架构，即非一致性内存访问架构。在这种架构下，每个CPU都有自己的本地Node，而其他CPU的Node就是远端Node。CPU访问本地Node的速度要比远端快很多。为了优化内存访问效率，Linux内核将物理内存划分为多个Node，每个Node又包含了许多Zone，Zone又被划分为page。
...
伙伴算法实现了以页为单位、大块内存的分配和释放，而在实际编程中，我们仅仅需要分配和释放单个对象，因此slab分配器应运而生。slab分配器把不同的对象划分为{==高速缓存==}，其中每个高速缓存专门用于存储一种特定类型的对象。例如，一个高速缓存可能用于管理进程描述符（`task_struct`结构），而另一个则用于索引节点对象（`struct inode`）。当系统需要新的数据结构实例时，可以直接从高速缓存中获取一个现成的块，这样就避免了新内存的消耗。使用完毕后，该数据结构实例被归还到高速缓存中，而不是被直接释放。在这种机制下，高速缓存实际上是那些频繁被使用的数据结构类型的集合。这样的设计不仅提升了内存管理的效率，还减少了内存碎片，是一种非常实用的编程技巧。
