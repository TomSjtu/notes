...
要想中断处理程序运行得又快，完成的任务又多是不现实的。于是中断处理的整个过程就分为上半部和下半部两个部分。通常所说的中断处理程序是指上半部——当接收到一个中断后，它立刻被执行，但只做有严格时限的工作。例如对接收到的中断进行应答等等。而稍后需要完成的工作则会被推迟到下半部去，在合适的时机执行。内核提供了多种下半部的机制，稍后会做说明。
